...
class IsActive
{
    ...
    public function handle(Request $request, Closure $next, ...$roles)
    {
        ...
        if (! $user->hasRole(Role::SUPER_ADMIN) && ! $user->hasRole(Role::TWP_ADMIN) && ! $user->hasRole(Role::TWP_USER)) {
            if (! $user->hasRole(Role::TWP_GUEST)) {
                $role = Role::where('name', Role::TWP_GUEST)->first();
