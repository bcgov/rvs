<template>
    ...
    <div v-for="cat in categories" class="card mb-3">
        <div class="card-header">
            <div>{{ cat }}
                <button @click="newUtil(cat)" type="button" class="btn btn-success btn-sm float-end">New {{ cat }}</button>
            </div>
        </div>
        <div class="card-body">
...
export default {
    ...
    data() {
        return {
            ...
            newUtilFormData: {
                formState: '',
                field_name: '', field_type: '', active_flag: false,
            },
            editUtilForm: '',
        }
    },
    methods: {
        ...
        submitEditUtil: function () {
            this.editUtilForm.formState = '';
            this.editUtilForm.put('/plsc/maintenance/utils/' + this.editUtilForm.id, {
                onSuccess: (response) => {
        ...
        submitNewUtil: function ()
        {
            ...
            this.newUtilForm.post('/plsc/maintenance/utils', {
                onSuccess: (response) => {
